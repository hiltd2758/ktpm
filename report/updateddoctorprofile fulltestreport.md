...
**Kết quả tổng (toàn class `DoctorService` — method `updateDoctorProfile`):**

| Method                  |     Line Coverage | Branch Coverage | Cyclomatic Complexity |
| ----------------------- | ----------------: | --------------: | --------------------: |
| `updateDoctorProfile()` |       71% (17/24) |     46% (23/50) |                     7 |
| `getDoctorByEmail()`    |                0% |              0% |                     — |
| `mapToDTO()`            |                0% |               — |                     — |
| **TỔNG TOÀN CLASS**     | **24% (165/684)** | **46% (23/50)** |                     — |

> **Ghi chú:** Coverage 71% trên `updateDoctorProfile()` do nhánh `IOException` khi lưu file không được kích hoạt trong unit test (mock file không thực sự ghi xuống ổ đĩa). Các method `getDoctorByEmail()` và `mapToDTO()` nằm ngoài phạm vi test case này.
